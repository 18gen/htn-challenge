...
const EventList = () => {
  ...
  const [selectedTypes, setSelectedTypes] = useState<string[]>(['workshop', 'tech_talk', 'activity']);
  ...
  useEffect(() => {
    fetch('https://api.hackthenorth.com/v3/events')
      ...
      .then((data) => {
        const filteredEvents = data.filter((event: Event) => isAuthenticated || event.permission === 'public');
        setEvents(filteredEvents.sort((a: Event, b: Event) => a.start_time - b.start_time));
      })
      .catch((error) => console.error('Error fetching events:', error));
  }, [isAuthenticated]);

  const uniqueDates = useMemo(() => {
    return Array.from(new Set(events.map((event) => new Date(event.start_time).toDateString())));
  }, [events]);
  ...
  const filteredEvents = useMemo(() => {
    ...
    if (filterDate !== 'all') {
      filtered = filtered.filter((event) => new Date(event.start_time).toDateString() === filterDate);
    }

    if (selectedTypes.length > 0) filtered = filtered.filter((event) => selectedTypes.includes(event.event_type));
    else filtered = [];
      ...
      <div className="flex border-b border-gray-200 mt-1">
        <button
          className={`px-4 py-2 -mb-px text-sm font-medium border-b-2 transition-colors duration-300 ${
            viewType === 'list' ? 'border-blue-500 text-blue-500' : 'border-transparent text-gray-500 hover:text-gray-700'
          }`}
        ...
        <button
          className={`px-4 py-2 -mb-px text-sm font-medium border-b-2 transition-colors duration-300 ${
            viewType === 'calendar' ? 'border-blue-500 text-blue-500' : 'border-transparent text-gray-500 hover:text-gray-700'
          }`}
          onClick={() => {
          ...
          return (
            <button
              ...
              className={`flex items-center space-x-2 px-2.5 py-1 rounded border transition-colors duration-200 ${
                isSelected ? 'bg-white text-black border-gray-300' : 'bg-gray-200 text-gray-500 border-gray-200 line-through'
              }`}
            >
              <span
